...
async def main():
    ...
    # paths 可选：优先使用环境变量 SIRCHMUNK_SEARCH_PATHS，否则为当前工作目录
...
{{< cards >}}
  ...
  {{< card url="../guide/mcp" title="OpenClaw 集成" icon="puzzle-piece" subtitle="OpenClaw 配置见 MCP 指南" >}}
  {{< card url="../guide/docker" title="Docker 部署" icon="cloud" subtitle="容器化一键部署" >}}
{{< /cards >}}
