...
- **Claude** 通过 API 代理
- **MiniMax**、**DeepSeek**、**Moonshot**、**Mistral**、**Groq**、**Together AI**、**Cohere**、**Google Gemini**、**智谱（GLM）**、**百川**、**零一万物**、**硅基流动**、**火山引擎**、**Azure OpenAI**
- 其他任何 OpenAI 兼容供应商
...
不需要。`FILENAME_ONLY` 模式执行快速文件名搜索，不进行任何 LLM 调用。`FAST` 和 `DEEP` 模式均需要配置 LLM API 密钥。`FAST` 模式（默认）采用贪心策略和 early stopping，速度约为 `DEEP` 模式的 10 倍。
