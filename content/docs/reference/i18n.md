...
- **MiniMax**, **DeepSeek**, **Moonshot**, **Mistral**, **Groq**, **Together AI**, **Cohere**, **Google Gemini**, **Zhipu (GLM)**, **Baichuan**, **Yi**, **SiliconFlow**, **Volcengine**, **Azure OpenAI**
...
No. `FILENAME_ONLY` mode performs fast filename-based search without any LLM calls. Both `FAST` and `DEEP` modes require a configured LLM API key. `FAST` mode (default) uses a greedy strategy with early stopping for ~10x faster retrieval than `DEEP` mode.
